...
import { cn, formatBytes, formatBitrate, formatTime } from './utils';
    ...
    <div className="min-h-screen bg-[#fcfdfe] font-sans selection:bg-blue-100 selection:text-blue-900">
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto p-4 md:p-8 space-y-8">
        ...
        <div className="bg-white rounded-[40px] p-8 md:p-12 shadow-sm border border-slate-200 overflow-x-auto relative group">
          <div className="absolute top-8 left-12 text-[11px] font-black text-slate-300 uppercase tracking-[0.2em] pointer-events-none group-hover:text-blue-200 transition-colors">Path Visualization</div>
          
          <div className="min-w-[800px] flex items-center justify-between relative mt-8">
            
            {/* Base Path Line */}
            <div className="absolute top-1/2 left-10 right-10 h-1.5 bg-slate-100 -translate-y-1/2 z-0 rounded-full overflow-hidden shadow-inner">
               <motion.div 
                 initial={{ width: 0 }}
                 animate={{ 
                   width: story?.gnbDecision === 'ACCEPTED' ? '100%' : 
                          story?.profileId ? '66%' : 
            ...
            {[
              { id: 'ue', icon: Smartphone, label: 'Application', active: !!story?.phase },
              { id: 'sidecar', icon: Layers, label: 'UE Sidecar', active: story?.phase === 'prepared' || !!story?.profileId },
              { id: 'upf', icon: Settings, label: 'UPF Policy', active: !!story?.profileId },
              { id: 'gnb', icon: Wifi, label: 'gNB Assist', active: story?.gnbDecision === 'ACCEPTED' }
            ].map((node) => (
