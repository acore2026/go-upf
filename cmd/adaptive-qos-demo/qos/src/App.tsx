...
export default function App() {
  ...
  const flowActive = !!flow?.active || (sidecarStatus?.activeFlows || 0) > 0 || (upfStatus?.activeFlows || 0) > 0;
        ...
        <nav className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <button 
              ...
              disabled={isInjecting || !flowId || !flowActive}
...
function isRejectedStoryStart(resp: any) {
  ...
  if (status && ['rejected', 'error', 'failed', 'failure'].includes(status)) {
    return true;
  }

  const reasonCode = String(resp.reasonCode || resp.ReasonCode || '').toUpperCase();
  if (reasonCode && reasonCode !== 'ACCEPTED' && reasonCode !== 'OK') {
    return true;
  }
